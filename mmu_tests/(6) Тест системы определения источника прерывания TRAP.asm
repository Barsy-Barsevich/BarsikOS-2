...
; (6) Тест системы определения источника прерывания TRAP
;
; Данная программа представляет из себя системную функцию, вызываемую
; пользовательским процессом. Системная функция смотрит источник прерывания
; и осознает, что было источником. Если на экране загорается цифра '1', то
; источником прерывания был таймер, а если '2' - то процесс пользователя.
; Программа выдает ответ '2', так что источник прерывания - процесс.
; Крайне важно отметить, что пока идет обработка запроса системной функции,
; таймер при некоторых условиях может успеть сработать, и тогда программа
; выдаст результат, будто источником прерывания был таймер.

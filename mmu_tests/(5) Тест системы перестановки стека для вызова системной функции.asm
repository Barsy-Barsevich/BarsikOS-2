...
; (5) Тест системы перестановки стека для вызова системной функции
;
; Программа пользователя инкрементирует BCD-переменную, затем ждет 62мс.
; Потом вызывает функцию печати данных на дисплей. Функция 'Function' внутри
; себя вызывает 'SYS_OS_muutos', которая инициирует переход в режим ОС.
; Подпрограмма обработки прерывания по вектору TRAP делает перестановку стека,
; убирая адрес возврата на 'SYS_OS_muutos' и 'Function' из стека, а также
; внедряя в стек указатель на 'SYS_User_muutos'. Затем выполняется безусловный
; переход по адресу возврата 'Function'.
; На работоспособность программы указывает быстрая перемена цифер на дисплее
; вычислительной платы.

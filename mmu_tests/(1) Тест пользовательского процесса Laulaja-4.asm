...
; (1) Тест работы пользовательского процесса (16.11.2023)
;
; Данный тест создан, чтобы доказать возможность работы вычислительной платы
; в пользовательском режиме. В пользовательском режиме в цикле раз в 500мс
; увеличивается значение BCD-переменной. Подпрограмма обработки прерывания
; по вектору TRAP печатает значение переменной на экран. Раз в полсекунды
; значение на экране инкрементируется. Это доказывает, что вычислительная
; плата успешно переходит в режим пользователя и выполняет нужный процесс.
